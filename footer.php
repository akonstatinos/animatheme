...
$copyright_text = !empty($opt['copyright_txt']) ? $opt['copyright_txt'] : esc_html__( '© 2019 DroitThemes. All rights reserved', 'saasland' );
...
jQuery(document).ready(function() {
	jQuery(".menu-sportsbook a").on("click", function(e) {
		if (typeof ASb != 'undefined') {
			e.preventDefault();
			ASb.setParams({page: 'prelive'});
		}
		jQuery(".menu-sportsbook").addClass("active");
                jQuery(".menu-live").removeClass("active");
                jQuery(".menu-virtuals").removeClass("active");
	});
        jQuery(".menu-live a").on("click", function(e) {
		if (typeof ASb != 'undefined') {
			e.preventDefault();
	                ASb.setParams({page: 'live'});
		}
                jQuery(".menu-sportsbook").removeClass("active");
                jQuery(".menu-live").addClass("active");
                jQuery(".menu-virtuals").removeClass("active");
        });
        jQuery(".menu-virtuals a").on("click", function(e) {
		if (typeof ASb != 'undefined') {
			e.preventDefault();
                	ASb.setParams({page: 'virtuals'});
		}
                jQuery(".menu-sportsbook").removeClass("active");
                jQuery(".menu-live").removeClass("active");
                jQuery(".menu-virtuals").addClass("active");
        });
});
</script>
</body>
